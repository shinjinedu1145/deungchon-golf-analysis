...
/* ── Global Dark Theme ── */
.stApp { font-family: 'Inter', -apple-system, sans-serif; background: #0b1120 !important; color: #e2e8f0 !important; }
.main .block-container { padding: 0.3rem 2rem 2rem 2rem; max-width: 1500px; }
.main { background: #0b1120 !important; }
header[data-testid="stHeader"] { background: rgba(11,17,32,0.95) !important; }
p, span, label, .stMarkdown { color: #cbd5e1 !important; }
h1, h2, h3, h4, h5, h6 { color: #f1f5f9 !important; }

/* ── Sidebar ── */
section[data-testid="stSidebar"] { background: linear-gradient(180deg, #070d1a 0%, #111827 100%); width: 300px; border-right: 1px solid #1e293b; }
section[data-testid="stSidebar"] * { color: #94a3b8 !important; }
section[data-testid="stSidebar"] h1, section[data-testid="stSidebar"] h2,
section[data-testid="stSidebar"] h3, section[data-testid="stSidebar"] h4,
section[data-testid="stSidebar"] h5 { color: #e2e8f0 !important; }
section[data-testid="stSidebar"] .stNumberInput input,
section[data-testid="stSidebar"] .stTextInput input,
section[data-testid="stSidebar"] .stSelectbox > div > div { background: #0f172a; border: 1px solid #334155; color: #e2e8f0 !important; border-radius: 6px; font-size: 0.85rem; }
section[data-testid="stSidebar"] details { background: #0f172a; border: 1px solid #1e293b; border-radius: 8px; margin-bottom: 6px; }
section[data-testid="stSidebar"] details summary { font-weight: 600; font-size: 0.88rem; padding: 10px 12px; color: #e2e8f0 !important; }
section[data-testid="stSidebar"] details[open] { border-color: #3b82f6; }
...
div[data-testid="stTabs"] > div:first-child {
    ...
    padding: 6px 4px 2px 4px; border-bottom: 1px solid #1e293b;
}
div[data-testid="stTabs"] > div:first-child > div {
    flex-wrap: wrap !important; gap: 3px 4px !important;
}
div[data-testid="stTabs"] button[data-baseweb="tab"] {
    font-size: 12.5px; font-weight: 600; padding: 7px 12px; color: #64748b !important;
    border-radius: 6px 6px 0 0; white-space: nowrap; background: #111827 !important;
...
div[data-testid="stTabs"] button[data-baseweb="tab"][aria-selected="true"] {
    color: #60a5fa !important; background: rgba(59,130,246,0.15) !important;
    border-color: #3b82f6; box-shadow: inset 0 -2px 0 #3b82f6;
}
...
div[data-testid="stMetricLabel"] { font-size: 0.72rem !important; font-weight: 600 !important; color: #64748b !important; text-transform: uppercase; letter-spacing: 0.05em; }
div[data-testid="stMetricValue"] { font-size: 1.6rem !important; font-weight: 900 !important; color: #f1f5f9 !important; }
...
div[data-testid="stDataFrame"] th { background: #1e293b !important; color: #e2e8f0 !important; font-weight: 600 !important; }
div[data-testid="stDataFrame"] td { background: #111827 !important; color: #cbd5e1 !important; border-color: #1e293b !important; }
div[data-testid="stDataFrame"] tr:hover td { background: #1e293b !important; }

/* ── Alerts (Dark) ── */
div[data-testid="stAlert"] { border-radius: 8px; background: #111827 !important; }

/* ── Section Headers ── */
.sec { background: linear-gradient(135deg, #1e293b, #0f172a); color: #e2e8f0; padding: 14px 22px; border-radius: 8px; margin: 24px 0 14px 0; font-size: 1rem; font-weight: 700; border-left: 4px solid #3b82f6; letter-spacing: -0.01em; }
.sec span { font-size: 1.1rem; margin-right: 6px; }
.sub-sec { background: transparent; border-left: 3px solid #60a5fa; padding: 10px 16px; border-radius: 0; margin: 18px 0 10px 0; font-weight: 600; color: #93c5fd; font-size: 0.95rem; }
...
/* ── Expanders in main ── */
details { background: #111827 !important; border: 1px solid #1e293b !important; border-radius: 8px !important; }
...
/* ── Footer ── */
.footer { background: linear-gradient(135deg, #070d1a, #111827); color: #475569; padding: 14px 24px; border-radius: 8px; text-align: center; font-size: 0.78rem; margin-top: 28px; border: 1px solid #1e293b; }
...
④ 6월 오픈 = 장마+폭염 → 체험 방문 자체가 적음
    ...
    with st.expander("🏗️ 월 고정 운영비"):
        ...
        op_tax = st.number_input("세금과공과 (만원/월)", value=sv('op_tax', 6890), step=100, key='op_tax',
            help="Excel 2026F: 세금과공과 6.20억/9개월 = 월 6,891만. 재산세+종토세+부가세 등")
    ...
    with st.expander("📍 상권·경쟁 가중치"):
        st.caption("상권 입지 평가 (100점 만점)")
        tw_pop = st.slider("배후인구 규모", 0, 100, sv('tw_pop', 75), 5, key='tw_pop')
        tw_access = st.slider("접근성 (교통/주차)", 0, 100, 80, 5, key='tw_acc')
        tw_visible = st.slider("가시성/노출도", 0, 100, sv('tw_vis', 70), 5, key='tw_vis')
        tw_compete = st.slider("경쟁강도 (높을수록 약)", 0, 100, 55, 5, key='tw_comp')
        tw_growth = st.slider("지역성장성 (마곡 등)", 0, 100, 85, 5, key='tw_grow')
        st.markdown("---")
        st.caption("가중치 설정 (합계 100%)")
        w_pop = st.number_input("배후인구 가중치", value=sv('w_pop', 25), step=5, key='w_pop')
        w_acc = st.number_input("접근성 가중치", value=sv('w_acc', 20), step=5, key='w_acc')
        w_vis = st.number_input("가시성 가중치", value=sv('w_vis', 15), step=5, key='w_vis')
        w_comp = st.number_input("경쟁강도 가중치", value=sv('w_comp', 20), step=5, key='w_comp')
        w_grow = st.number_input("성장성 가중치", value=sv('w_grow', 20), step=5, key='w_grow')
        st.markdown("---")
        st.caption("경쟁사 수 (반경 3km)")
        s_comp_outdoor = st.number_input("실외 연습장", value=sv('s_co', 2), step=1, key='s_co')
        s_comp_indoor = st.number_input("실내 연습장", value=sv('s_ci', 5), step=1, key='s_ci')
        s_comp_screen = st.number_input("스크린골프", value=sv('s_cs', 12), step=1, key='s_cs')

    with st.expander("📉 골프시장·경제지표"):
        # API 자동수집 연동
        st.caption("🔄 자동 수집 설정")
        ...
        src_emoji = "🟢" if src_label == "api" else "🟡" if src_label == "mixed" else "⚪"
        st.caption(f"{src_emoji} {ed.get('last_update', '기본값')}")

        st.markdown("---")
        st.caption("골프 산업 환경 변수")
        st.caption("ℹ️ 한국골프장경영협회, 문체부 참여실태조사 기준. 연습장은 실내/실외로 구분되며, 실외는 규모별로 시장 포지션이 다릅니다.")
        e_golf_pop = st.number_input("강서구 골퍼 인구 (천명)", value=sv('e_gpop', 388), step=10, key='e_gpop', help="Excel '골프시장 분석' 시트: 강서구 골퍼 443천(2018)→388천(2025). 골프장이 아닌 골프연습장 기준")
        e_golf_growth = st.number_input("골프시장 하락률 (%)", value=sv('e_ggrow', float(A['market']['골프시장하락률'][1]*100)), step=0.5, key='e_ggrow', help="Excel ① 가정 시트 Base: -3.5%. 실외 연습장 시장 2023년 -14.4%, 2024년 -15.2%") / 100
        st.markdown("---")
        st.caption("📊 전국 연습장 현황 (실내/실외)")
        e_indoor_count = st.number_input("실내 연습장 수", value=sv('e_indoor', 650), step=10, key='e_indoor', help="인도어/실내 연습장 (시뮬레이터 제외)")
        e_outdoor_large = st.number_input("실외 대형 (80타석+)", value=sv('e_out_l', 280), step=10, key='e_out_l', help="80타석 이상 대형 연습장. 등촌은 88타석으로 대형에 해당")
        e_outdoor_mid = st.number_input("실외 중형 (40~79타석)", value=sv('e_out_m', 450), step=10, key='e_out_m', help="40~79타석 중형 연습장")
        e_outdoor_small = st.number_input("실외 소형 (40타석 미만)", value=sv('e_out_s', 470), step=10, key='e_out_s', help="40타석 미만 소형 연습장")
        e_range_count = e_indoor_count + e_outdoor_large + e_outdoor_mid + e_outdoor_small
        st.caption(f"**전국 연습장 합계: {e_range_count:,}개** (실내{e_indoor_count}+실외{e_outdoor_large+e_outdoor_mid+e_outdoor_small})")
        e_screen_count = st.number_input("전국 스크린골프 수", value=sv('e_scnt', 9500), step=100, key='e_scnt', help="골프존/카카오VX 등 스크린골프 매장")
        st.markdown("---")
        st.caption("경제 지표")
        e_gdp_growth = st.number_input("GDP 성장률 (%)", value=sv('e_gdp', float(ed.get('gdp_growth', 2.1))), step=0.1, key='e_gdp') / 100
        e_cpi = st.number_input("소비자물가 상승률 (%)", value=sv('e_cpi', float(ed.get('cpi_rate', 2.5))), step=0.1, key='e_cpi') / 100
        e_interest = st.number_input("기준금리 (%)", value=sv('e_ir', float(ed.get('base_rate', 3.0))), step=0.25, key='e_ir') / 100
        e_unemp = st.number_input("실업률 (%)", value=sv('e_unemp', float(ed.get('unemployment', 3.5))), step=0.1, key='e_unemp') / 100
        e_disposable = st.number_input("가처분소득 증감 (%)", value=sv('e_disp', float(ed.get('disposable_income_growth', 1.5))), step=0.5, key='e_disp') / 100
        st.markdown("---")
        st.caption("영향도 가중치 (합계 100%)")
        ew_golf = st.number_input("골프시장 가중치", value=sv('ew_g', 30), step=5, key='ew_g')
        ew_gdp = st.number_input("GDP 가중치", value=sv('ew_gdp', 20), step=5, key='ew_gdp')
        ew_cpi = st.number_input("물가 가중치", value=sv('ew_cpi', 15), step=5, key='ew_cpi')
        ew_ir = st.number_input("금리 가중치", value=sv('ew_ir', 15), step=5, key='ew_ir')
...
econ_golf_score = max(0, min(100, 50 + e_golf_growth * 1000))
econ_gdp_score = max(0, min(100, e_gdp_growth * 2000))
econ_cpi_score = max(0, min(100, 100 - e_cpi * 2000))
econ_ir_score = max(0, min(100, 100 - e_interest * 1000))
econ_disp_score = max(0, min(100, 50 + e_disposable * 1000))
if ew_total > 0:
...
ta_rev_adj = ta_score / 70  # 70점 기준 = 1.0x

# Economic impact score
ew_total = ew_golf + ew_gdp + ew_cpi + ew_ir + ew_disp
# Normalize each indicator to 0-100 scale (positive = good for business)
econ_golf_score = max(0, min(100, 50 + e_golf_growth * 1000))  # -5%→0, 0%→50, +5%→100
econ_gdp_score = max(0, min(100, e_gdp_growth * 2000))  # 0%→0, 5%→100
econ_cpi_score = max(0, min(100, 100 - e_cpi * 2000))  # 0%→100, 5%→0 (inflation bad)
econ_ir_score = max(0, min(100, 100 - e_interest * 1000))  # 0%→100, 10%→0 (high rate bad)
econ_disp_score = max(0, min(100, 50 + e_disposable * 1000))  # -5%→0, 0%→50, +5%→100
if ew_total > 0:
    econ_score = (econ_golf_score*ew_golf + econ_gdp_score*ew_gdp + econ_cpi_score*ew_cpi + econ_ir_score*ew_ir + econ_disp_score*ew_disp) / ew_total
else:
    econ_score = 50
econ_grade = "호황" if econ_score >= 65 else "보통" if econ_score >= 45 else "불황"
econ_rev_adj = econ_score / 55  # 55점 기준 = 1.0x
...
if _ti == 0:
    ...
    k = st.columns(7)
    k[0].metric("투자금", f"{s_inv}억")
    k[1].metric("NPV", fmt억(npv_val))
    k[2].metric("누적EBITDA", fmt억(cum_ebitda[-1]))
    k[3].metric("회수율", f"{rec_rate[-1]*100:.1f}%")
    k[4].metric("IRR", f"{irr_val*100:.1f}%")
    k[5].metric("BEP매출", fmt억(bep_revenue))
    k[6].metric("Payback", f"{payback:.1f}년" if payback else "5년+")
    ...
    with col_r:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            gauge={
                'axis': {'range': [0, 150], 'tickfont': {'size': 10, 'color': '#64748b'}},
...
if _ti == 1:
    ...
    for name, rev_adj, cost_adj, dr in scen_params:
        ...
        s_dr = dr / 100
        s_npv = sum(e/(1+s_dr)**(i+1) for i,e in enumerate(s_ebitda)) - inv_won
        try:
            s_irr = float(np_irr([-inv_won] + s_ebitda))
        except Exception:
            s_irr = (sum(s_ebitda)/5) / inv_won if inv_won else 0
    ...
    with ctab1:
        if D['contrib']:
            ...
            for i, yr in enumerate(years_avail):
                ...
                yk[i].metric(f"{yr}년 이익", f"{yr_profit/만:,.0f}만", delta=f"영업이익률 {yr_margin:.1f}%")
            ...
            for yr in years_avail:
                ...
                disp_c = dfy[['상품','회원수','매출','추정비용','이익','매출비중','이익기여도','이익률']].copy()
...
""", unsafe_allow_html=True)
    ...
    with ctab2:
        ...
        info("상품별 **회원수**와 **평균단가**를 입력하면, 과거 분석과 동일한 구조(상품별·유형별·성별·고정/변동·요일시간대)로 자동 분석됩니다.")

        # ── 입력 테이블 ──
        st.caption("📋 상품별 연간 회원수 · 평균단가 · 비용 입력")
        f_items = [
            ('1개월권', 500, 280000, 65), ('3개월권', 1000, 760000, 60),
            ('6개월권', 80, 1500000, 55), ('쿠폰', 1800, 400000, 60),
            ('일일권', 35000, 22000, 70), ('락카', 1200, 240000, 5),
            ('프로임대', 3, 18000000, 0),
        ]
...
""", unsafe_allow_html=True)
    ...
    # ── AI 추천 전체 요금표 (제니스 비교 포함) ──
    subsec("AI 추천 등촌골프연습장 전체 요금표")
    info("제니스(구로구 고척동, 112타석) 대비 3~5% 저렴하되, '저렴해서 온다'가 아닌 '비슷한 가격에 신축 시설이라 온다'는 인식. 1~2년차 동결 → 3년차 물가분 +3% 인상. 모든 금액 만원 단위, VAT 포함.")
...
<th style="background:#0f4c3a;color:#86efac;padding:8px;text-align:center;border-bottom:1px solid #334155;" colspan="2">AI 추천가 (1~2년차, 동결)</th>
<th style="background:#1e3a5f;color:#93c5fd;padding:8px;text-align:center;border-bottom:1px solid #334155;" colspan="2">3년차~ (물가 +3% 인상)</th>
<th style="background:#4a1d1d;color:#fca5a5;padding:8px;text-align:center;border-bottom:1px solid #334155;" colspan="2">제니스 (비교)</th>
<th style="background:#1e293b;color:#fbbf24;padding:8px;text-align:center;border-bottom:2px solid #334155;" rowspan="2">할인율</th>
</tr>
<tr>
...
<th style="background:#4a1d1d;color:#fca5a5;padding:6px;text-align:center;border-bottom:2px solid #334155;">남</th>
<th style="background:#4a1d1d;color:#fca5a5;padding:6px;text-align:center;border-bottom:2px solid #334155;">여</th>
</tr>"""

    # ── 이익기여도 + 상권 특성 기반 전략적 가격 설계 ──
    #
    # [핵심 팩트]
    # 1. 제니스는 구로구 고척동 → 여의도·영등포 고소득 직장인 고객 다수, 가격 민감도 낮음
    # 2. 등촌은 강서구 → 마곡지구 발전 중이나, 주거지역 중심 고객층, 가격 민감도 상대적 높음
    # 3. 따라서 제니스 가격을 그대로 따라가면 등촌 고객에겐 부담 → 차별화된 할인 설계 필요
    #
    # [이익기여도 기반 할인 전략]
    # 일일/쿠폰(이익기여도 1~2위): 1방문당 수익 최고 → 유입채널, 할인 최대(-4~4.5%)
    # 1개월(3위): 진입 상품 → 적정 할인(-3~4%) → 3개월 전환 유도
    # 3개월(4위): 안정 매출 → 볼륨 확보 할인(-3.5~4.5%)
    # 6개월(5위): 객단가 최저+사우나/헬스 무료 비용 → 할인 최소(-2~3%)
    #
    # 만원 단위 올림, 제니스 대비 최대 -4.5%
    # 1~2년차 동결 → 3년차 물가분 +3% 인상
    # 전 항목 제니스 대비 최대 -4.5% 이하, 만원 단위 올림
    rows = [
        ('1개월', '모닝(10시이전)', '250,000', '230,000', '260,000', '240,000', '260,000', '240,000', '-3.8%'),
        ('', '종일', '290,000', '260,000', '300,000', '270,000', '300,000', '270,000', '-3.3%'),
        ('', '자유(주말포함)', '330,000', '300,000', '340,000', '310,000', '340,000', '310,000', '-2.9%'),
        ('3개월', '모닝(10시이전)', '670,000', '630,000', '690,000', '650,000', '700,000', '660,000', '-4.3%'),
        ('', '종일', '800,000', '700,000', '830,000', '720,000', '830,000', '730,000', '-3.6%'),
        ('', '자유(주말포함)', '920,000', '840,000', '950,000', '870,000', '960,000', '880,000', '-4.2%'),
        ('6개월', '모닝(10시이전)', '1,390,000', '1,220,000', '1,430,000', '1,260,000', '1,450,000', '1,270,000', '-4.1%'),
        ('', '종일', '1,560,000', '1,360,000', '1,610,000', '1,400,000', '1,630,000', '1,420,000', '-4.3%'),
        ('', '자유(주말포함)', '1,740,000', '1,660,000', '1,790,000', '1,710,000', '1,820,000', '1,730,000', '-4.4%'),
    ]

    for i, (cat, time, a1, a2, b1, b2, c1, c2, disc) in enumerate(rows):
        bg = '#111827' if i % 2 == 0 else '#0f172a'
...
<td style="background:{bg};color:#86efac;padding:8px;text-align:right;border-bottom:1px solid #1e293b;font-weight:600;">{a1}</td>
<td style="background:{bg};color:#86efac;padding:8px;text-align:right;border-bottom:1px solid #1e293b;">{a2}</td>
<td style="background:{bg};color:#93c5fd;padding:8px;text-align:right;border-bottom:1px solid #1e293b;">{b1}</td>
<td style="background:{bg};color:#93c5fd;padding:8px;text-align:right;border-bottom:1px solid #1e293b;">{b2}</td>
<td style="background:{bg};color:#fca5a5;padding:8px;text-align:right;border-bottom:1px solid #1e293b;">{c1}</td>
<td style="background:{bg};color:#fca5a5;padding:8px;text-align:right;border-bottom:1px solid #1e293b;">{c2}</td>
...
<th style="background:#0f4c3a;color:#86efac;padding:10px;text-align:right;border-bottom:2px solid #334155;">AI 추천가 (1년차)</th>
<th style="background:#1e3a5f;color:#93c5fd;padding:10px;text-align:right;border-bottom:2px solid #334155;">정상가 (2년차~)</th>
<th style="background:#4a1d1d;color:#fca5a5;padding:10px;text-align:right;border-bottom:2px solid #334155;">제니스</th>
<th style="background:#1e293b;color:#fbbf24;padding:10px;text-align:center;border-bottom:2px solid #334155;">할인율</th>
<th style="background:#1e293b;color:#94a3b8;padding:10px;text-align:left;border-bottom:2px solid #334155;">전략 근거</th>
</tr>"""

    # 쿠폰: 회원권 전환 브릿지 (-3~4.5%)
    # 일일권: 최초 유입채널, 할인 최대 (-4~4.5%) → 체험 후 회원 전환 핵심
    etc_rows = [
        ('쿠폰', '10회(70분)', '220,000', '230,000', '230,000', '-4.3%', '체험 진입, 20회 전환 유도'),
        ('', '20회(70분)', '430,000', '440,000', '450,000', '-4.4%', '주력 쿠폰, 회당 21,500원'),
        ('', '30회(70분)', '600,000', '620,000', '620,000', '-3.2%', '충성고객, 회당 20,000원'),
        ('일일권', '주중 70분', '23,000', '24,000', '24,000', '-4.2%', '최초 유입, 회원전환 핵심'),
        ('', '주중 90분', '29,000', '30,000', '30,000', '-3.3%', '장시간 수요 흡수'),
        ('', '주말 70분', '26,000', '26,000', '27,000', '-3.7%', '주말 가동률 확보'),
        ('', '주말 90분', '32,000', '32,000', '33,000', '-3.0%', '주말 프리미엄'),
        ('락카', '보증금', '20,000', '20,000', '20,000', '동일', '업계 표준'),
        ('', '월 사용료', '20,000', '20,000', '20,000', '동일', '순수익 (원가 거의 없음)'),
    ]

    for i, (cat, detail, a, b, c, disc, reason) in enumerate(etc_rows):
        bg = '#111827' if i % 2 == 0 else '#0f172a'
...
<td style="background:{bg};color:#86efac;padding:8px;text-align:right;border-bottom:1px solid #1e293b;font-weight:600;">{a}</td>
<td style="background:{bg};color:#93c5fd;padding:8px;text-align:right;border-bottom:1px solid #1e293b;">{b}</td>
<td style="background:{bg};color:#fca5a5;padding:8px;text-align:right;border-bottom:1px solid #1e293b;">{c}</td>
<td style="background:{bg};color:#fbbf24;padding:8px;text-align:center;border-bottom:1px solid #1e293b;font-weight:600;">{disc}</td>
...
""")
    ...
    fc[5].metric("종합 보정계수", f"{combined:.2f}배")
...
if _ti == 5:
    ...
    c1, c2 = st.columns(2)
    with c1:
        info("2027년 기준 비용 항목별 구성 비율입니다. 인건비, 임대료 등 고정비 비중이 높으면 손익분기 달성이 어려워집니다.")
        cl = list(D['cost_items'].keys()); cv = [D['cost_items'][k][1] for k in cl]
    ...
    subsec("비용 항목별 상세")
    info("5개년 비용 항목별 상세 금액표입니다. 각 항목의 연도별 증감을 확인하세요.")
    cd = {'항목': list(D['cost_items'].keys())}
    for i, yr in enumerate(D['yp']): cd[yr] = [fmt만(D['cost_items'][k][i])+'원' for k in cd['항목']]
...
if _ti == 6:
    ...
    subsec("월별 매출 항목별 분해")
    info("2026년 월별 매출을 상품별로 분해한 누적 막대 차트입니다. 시즌별 매출 구성 변화를 확인할 수 있습니다.")
    fig_ms = go.Figure()
    for idx, (name, vals) in enumerate(D['monthly_rev_items'].items()):
        fig_ms.add_trace(go.Bar(name=name, x=D['months'], y=[v/1e6 for v in vals], marker_color=PAL[idx%len(PAL)]))
    lo(fig_ms, title='월별 매출 항목별 분해 (백만원)', barmode='stack', height=420, yaxis_title='백만원')
...
if _ti == 7:
    ...
    with c1:
        ...
        for i, yr in enumerate(D['yp']):
            ...
            dcf.append({'연도': yr, 'EBITDA': fmt억(ebitda_p[i]), '할인계수': f"{1/(1+disc_r)**(i+1):.4f}", '현재가치': fmt억(pv)})
        dcf.append({'연도': '합계', 'EBITDA': fmt억(sum(ebitda_p)), '할인계수': '-', '현재가치': fmt억(sum(ebitda_p[i]/(1+disc_r)**(i+1) for i in range(5)))})
    ...
    subsec("할인율별 NPV 비교")
    info("할인율(자본비용)을 변동시켰을 때 NPV의 변화를 비교합니다. NPV가 0이 되는 할인율이 IRR과 일치합니다.")
    dr_range = [0.06,0.08,0.10,0.12,0.15,0.18,0.20]
    npv_by_dr = [sum(e/(1+dr)**(i+1) for i,e in enumerate(ebitda_p))-inv_won for dr in dr_range]
...
if _ti == 8:
    sec("💰", "현금흐름 분석")
    subsec("5개년 현금흐름표 (임대수익 포함)")
    info("EBITDA에 임대수익을 더하고 법인세를 차감하여 영업 현금흐름(OCF)과 잉여 현금흐름(FCF)을 산출합니다. 누적 FCF가 양(+)이면 현금이 축적되고 있음을 의미합니다.")
    cf_data = {'항목': ['EBITDA','임대수익(+)','법인세(-)','영업CF','FCF','누적FCF']}
    cum_fcf = 0
    for i, yr in enumerate(D['yp']):
        rent_inc = total_rental_income[i] if i < len(total_rental_income) else 0
        tax = max(op_p[i]*s_tax_rate,0)
        ocf = ebitda_p[i]+rent_inc-tax
        fcf = ocf; cum_fcf += fcf
        cf_data[yr] = [fmt억(ebitda_p[i]), fmt억(rent_inc), fmt억(tax), fmt억(ocf), fmt억(fcf), fmt억(cum_fcf)]
    dark_table(pd.DataFrame(cf_data))

    c1, c2 = st.columns(2)
    with c1:
        subsec("FCF 추이 (임대수익 포함)")
...
if False:  # Tab removed from navigation
    ...
    labor_ratio = D['cost_items']['인건비'][1] / rev_p[1] * 100 if rev_p[1] else 0
    energy_ratio = (D['cost_items']['전력비'][1] + D['cost_items']['수도광열비'][1]) / rev_p[1] * 100 if rev_p[1] else 0
...
if _ti == 12:
    ...
    st.markdown("**보정계수 적용**")
    adj_data = {
        '보정 항목': ['시즌 가중치 평균', '오픈 램프업 평균', '상권 보정', '경제 보정', '종합 (2026F)', '종합 (2027F~, 램프업 제외)'],
        '계수': [f'{_season_avg:.3f}', f'{_ramp_avg:.3f}', f'{ta_rev_adj:.3f}', f'{econ_rev_adj:.3f}', f'{_adj_2026:.3f}', f'{_adj_normal:.3f}'],
        '의미': [f'비수기 반영 → 정상의 {_season_avg*100:.1f}%', f'오픈 초기 유입 → 정상의 {_ramp_avg*100:.1f}%',
                f'상권점수 {ta_score:.0f}/70 기준', f'경제점수 {econ_score:.0f}/55 기준',
                f'시즌×램프업×상권×경제', f'시즌×상권×경제 (2년차부터 정상)'],
    }
    ...
    calc_2026 = {
        ...
        '산식': ['회원수×단가 합계', f'외부{s_rent_external}+계열사{s_rent_affiliate}×4개월', '골프+임대',
                '시즌×램프업×상권×경제', f'{_golf_raw/억:.2f}×{_adj_2026:.3f}', '보정 골프 + 임대'],
    }
    ...
    # ── 7. 보정계수 영향 시뮬레이션 ──
    subsec("7. 보정계수별 매출 영향")
    sim_data = {
        '시나리오': ['보정 없음 (이론치)', '시즌만 적용', '시즌+램프업', '시즌+램프업+상권', '전체 보정 (현재)'],
        '2026F 매출': [
            f'{_golf_raw/억:.1f}억',
            f'{_golf_raw*_season_avg/억:.1f}억',
            f'{_golf_raw*_season_avg*_ramp_avg/억:.1f}억',
            f'{_golf_raw*_season_avg*_ramp_avg*ta_rev_adj/억:.1f}억',
            f'{rev_p[0]/억:.1f}억',
        ],
        '보정률': ['100%', f'{_season_avg*100:.1f}%', f'{_season_avg*_ramp_avg*100:.1f}%',
                  f'{_season_avg*_ramp_avg*ta_rev_adj*100:.1f}%', f'{_adj_2026*100:.1f}%'],
    }
    dark_table(pd.DataFrame(sim_data))
